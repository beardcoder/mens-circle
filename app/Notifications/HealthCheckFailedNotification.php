    ...
    public function shouldSend(object $notifiable, string $channel): bool
    {
        ...
        try {
            if ($lock->get()) {
                ...
                $lastNotificationSentAtTimestamp = is_numeric($lastNotificationSentAt)
                    ? (int) $lastNotificationSentAt
                    : null;

                if (
                    $lastNotificationSentAtTimestamp === null
                    || (now()->getTimestamp() - $lastNotificationSentAtTimestamp) >= ($throttleMinutes * 60)
                ) {
                    Cache::put($cacheKey, now()->getTimestamp(), $throttleMinutes * 60);
