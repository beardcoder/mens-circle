...
test('resolve replaces placeholders with event data', function (): void {
    ...
    expect($result['subject'])->toContain('Männerkreis Januar')
        ->and($result['content'])->toContain('Männerkreis Januar')
        ->and($result['content'])->toContain('Straubing')
        ->and($result['content'])->toContain('19:00')
        ->and($result['content'])->toContain('Auf Spendenbasis')
        ->and($result['content'])->toContain('8 Plätze');
});
...
test('resolve uses next event when no event provided', function (): void {
    ...
    expect($result['subject'])->toContain('Nächstes Treffen')
        ->and($result['content'])->toContain('München');
});
...
test('replace placeholders works with custom strings', function (): void {
    ...
    $service = new EmailTemplateService();
    $result = $service->replacePlaceholders(
        'Einladung: {event_title}',
        '<p>Treffen in {event_location} am {event_date}</p>',
        $event,
    );

    expect($result['subject'])->toContain('Custom Event')
        ->and($result['content'])->toContain('Regensburg');
});
